...
import { tools, predefine, MixinsState } from '../object-plus';
...
export function attr(proto, attrName) {
    if (attrName) {
        if (typeof Reflect !== 'undefined') {
            var attrSpec = Reflect.getMetadata("design:type", proto, attrName);
            injectAttribute(proto, attrName, attrSpec);
        }
        else {
            proto.log('error', 'Add import "reflect-metadata"; as the first line of your app.');
        }
    }
    else {
        var attrSpec_1 = proto;
        return function (proto, attrName) { return injectAttribute(proto, attrName, attrSpec_1); };
    }
}
