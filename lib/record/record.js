...
import { define, definitions, mixinRules, tools, log, isProduction } from '../object-plus';
import { Transactional } from '../transactions';
import { AggregatedType, AnyType, setAttribute, shouldBeAnObject, UpdateRecordMixin } from './attributes';
...
var Record = (function (_super) {
    ...
    function Record(a_values, a_options) {
        ...
        isProduction || typeCheck(_this, values);
    ...
    Record.prototype._log = function (level, topic, text, props) {
        log(level, 'record:' + topic, text, tslib_1.__assign({}, props, { 'Record': this, 'Attributes definition:': this._attributes }));
    };
...
function typeCheck(record, values) {
    if (shouldBeAnObject(record, values)) {
        ...
        if (unknown) {
            record._log('warn', 'typeError', "undefined attributes " + unknown.join(', ') + " are ignored.", { values: values });
        }
    }
}
